...
{
    private const ROUTE = 'comment.update';
    ...
    public function test_expected_unauthenticaded()
    {
        ...
        $response = $this->putJson(route(self::ROUTE, $this->comment->getKey()), $request);
    ...
    {
        $response = $this->actingAs($this->user)->putJson(route(self::ROUTE, $this->comment->getKey(), []));
    ...
    public function test_expected_unprocessable_entity_exception_when_description_bigger_than_100()
    {
        ...
        $response = $this->actingAs($this->user)->putJson(route(self::ROUTE, $this->comment->getKey()), $request);
